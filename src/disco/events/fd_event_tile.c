...
typedef struct fd_event_tile_in fd_event_tile_in_t;
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_event_client_align(),  fd_event_client_footprint()     );
...
during_frag( fd_event_tile_t * ctx,
  ...
  switch( ctx->in_kind[ in_idx ] ) {
    case IN_KIND_SHRED:
      fd_memcpy( ctx->shred_buf, src, sz );
      break;
    case IN_KIND_DEDUP:
    case IN_KIND_GENESI:
    case IN_KIND_IPECHO:
...
after_frag( fd_event_tile_t *   ctx,
            ...
            ulong               tspub,
            fd_stem_context_t * stem ) {
  (void)seq; (void)tsorig; (void)stem;

  switch( ctx->in_kind[ in_idx ] ) {
    case IN_KIND_SHRED: {
      FD_TEST( sz<=FD_NET_MTU );
      uchar * buffer = fd_circq_push_back( ctx->circq, alignof(fd_event_t), offsetof(fd_event_t, event)+sizeof(fd_event_shred_t)+sz );
      FD_TEST( buffer ); /* shred should be guaranteed to fit, since sz<=FD_NET_MTU */
      fd_event_t * event = fd_type_pun( buffer );
      event->tag = FD_EVENT_TAG_SHRED;
      event->timestamp_nanos = fd_frag_meta_ts_decomp( tspub, fd_tickcount() );

      fd_event_shred_protocol_t protocol;
      switch( fd_disco_netmux_sig_proto( sig ) ) {
        case DST_PROTO_SHRED:
          protocol = FD_EVENT_SHRED_PROTOCOL_TURBINE;
          break;
        case DST_PROTO_REPAIR:
          protocol = FD_EVENT_SHRED_PROTOCOL_REPAIR;
          break;
        default:
          FD_LOG_ERR(( "unexpected proto %lu in sig %lu", fd_disco_netmux_sig_proto( sig ), sig ));
      }

      event->event.shred = (fd_event_shred_t){
        .src.addr = fd_disco_netmux_sig_ip( sig ),
        .src.port = 0, /* TODO: Pass this along ... */
        .protocol = protocol,
        .payload_off = offsetof(fd_event_t, event) + sizeof(fd_event_shred_t),
        .payload_len = (ushort)sz,
      };
      /* TODO: Can remove copy by implementing allocate/commit model with fd_circq */
      fd_memcpy( (uchar *)event+event->event.shred.payload_off, ctx->shred_buf, sz );
      break;
    }
    case IN_KIND_DEDUP:
      /* currently no-op */
...
unprivileged_init( fd_topo_t *      topo,
  ...
  FD_SCRATCH_ALLOC_INIT( l, scratch );
  fd_event_tile_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_event_tile_t), sizeof(fd_event_tile_t)         );
  void * _event_client  = FD_SCRATCH_ALLOC_APPEND( l, fd_event_client_align(),  fd_event_client_footprint()     );
...
#define FD_EVENT_ID_SEED 0x812CAFEBABEFEEE0UL
  ...
  ulong boot_id = fd_hash( FD_EVENT_ID_SEED, _boot_id, 36UL );
  ...
  ulong machine_id = fd_hash( FD_EVENT_ID_SEED, _machine_id, 32UL );
  ...
  for( ulong i=0UL; i<tile->in_cnt; i++ ) {
    ...
    if( FD_LIKELY( !strcmp( link->name, "net_shred"         ) ) ) ctx->in_kind[ i ] = IN_KIND_SHRED;
...
fd_topo_run_tile_t fd_tile_event = {
  .name                     = "event",
  .rlimit_file_cnt          = 3UL, /* stderr, logfile, and socket to the server */
