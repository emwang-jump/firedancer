...
test_cursor_lifecycle( void ) {
  ...
  FD_TEST( !fd_circq_cursor_advance( circq ) );
  
  uchar * msg1 = fd_circq_push_back( circq, 1UL, 8UL );
  msg1[0] = 'A';
  msg1[7] = 'B';
  
  uchar const * out1 = fd_circq_cursor_advance( circq );
  ...
  uchar const * out2 = fd_circq_cursor_advance( circq );
  FD_TEST( out2[0]=='C' && out2[7]=='D' );
  FD_TEST( !fd_circq_cursor_advance( circq ) );
  
  fd_circq_reset_cursor( circq );
  out1 = fd_circq_cursor_advance( circq );
  out2 = fd_circq_cursor_advance( circq );
...
test_ack_protocol( void ) {
  ...
  ulong cursors[5];
  for( ulong i=0; i<5; i++ ) {
    uchar const * msg = fd_circq_cursor_advance( circq );
...
test_interleaved_ops( void ) {
  ...
  uchar const * m1 = fd_circq_cursor_advance( circq );
  uchar const * m2 = fd_circq_cursor_advance( circq );
  ...
  uchar const * m3 = fd_circq_cursor_advance( circq );
  uchar const * m4 = fd_circq_cursor_advance( circq );
  FD_TEST( m3[0]=='Y' && m4[0]=='Z' );
  
  FD_TEST( fd_circq_pop_until( circq, cursor_x )==0 );
  
  fd_circq_reset_cursor( circq );
  uchar const * first = fd_circq_cursor_advance( circq );
  FD_TEST( first[0]=='Y' );
}
...
test_stale_cursor_handling( void ) {
  ...
  fd_circq_reset_cursor( circq );
  msg = fd_circq_cursor_advance( circq );
  FD_TEST( msg );
}
...
test_cursor_sequence_monotonicity( void ) {
  ...
  for( ulong i=0; i<100; i++ ) {
    ...
    uchar const * out = fd_circq_cursor_advance( circq );
    if( out ) {
